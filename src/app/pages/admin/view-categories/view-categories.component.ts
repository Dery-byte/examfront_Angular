...
openAssignCourse(catId: number, templateRef: TemplateRef<any>): void {
  forkJoin({
    category: this._category.getCategory(catId),
    lecturers: this._category.getAllLecturers()
  }).subscribe({
    next: (result) => {
      this.categoryEdit = result.category;
    ...
    error: () => {
      this._snack.open('Failed to load category or lecturers', '', {
        duration: 3000
      });
    }
  });
...
assignCourse() {
  // Validate that a lecturer has been selected
  if (!this.categoryEdit.userId) {
    this._snack.open('Please select a lecturer', '', {
      duration: 3000
    });
    return;
  }
  ...
  this._category.assignCourseToLecturer(this.categoryEdit.cid, this.categoryEdit.userId)
    .subscribe({
      next: (response) => {
        this._snack.open('Course assigned successfully', '', {
          duration: 3000,
        ...
        // Refresh the categories list to show the updated lecturer
        this.getAllCategories();
      },
      error: (error) => {
        console.error('Error assigning course:', error);
        this._snack.open('Failed to assign course to lecturer', '', {
          duration: 3000,
  ...
  //   },
  //     (error) => {
  //       this._snack.open(" Could not load Lecturers! ", "", {
  //         duration: 3000,
  //       });
  //       this.login.logout();
  //       // console.log(error);
